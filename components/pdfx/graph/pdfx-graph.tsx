...
/**
 * compartido Y-axis grid lines y tick labels para cartesian charts (barra, line, area).
 * Extracted to evitar duplicating the identical block entre renderiza functions.
 */
  ...
  return (
    <>
      ...
      {xLabels.map((label, ci) => {
        ...
        return (
          // biome-ignore lint/suspicious/noArrayIndexKey: datos estáticos de gráfica PDF — el índice es una identidad estable
          <G key={`group-${ci}`}>
            {series.map((s, si) => {
      ...
      {data.map((d, i) => {
        ...
        return (
          ...
          <G key={`slice-${i}`}>
            ...
            {/* Mostrar etiqueta solo si el segmento es lo bastante grande */}
            {sweep > 15 && (
              <SvgText
                x={lp.x}
                y={lp.y + 3}
...
/**
 * PdfGraph — renders barra, horizontal-barra, line, area, pie, y donut charts
 * natively inside react-pdf documents using SVG primitives.
 *
 * No external chart libraries are required or usados — todas rendering is hecho via
 * react-pdf's built-en SVG soporte (`<Svg>`, `<Rect>`, `<Path>`, `<Line>`, etc.).
 *
 * @example barra chart
 * ```tsx
 * <PdfGraph
 ...
 * ```
 *
 * @example Donut chart con centro label
 * ```tsx
 * <PdfGraph
 *   variante="donut"
 *   datos={[
 ...
 * - No animations
 * - SVG texto inside charts uses SVG font attributes (no react-pdf StyleSheet fonts)
 * - para print PDFs usar SVG-friendly fonts registered con Font.register()
 */
